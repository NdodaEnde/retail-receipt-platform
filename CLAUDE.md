...
- **House brands & product attributes** (migration 006): `brand_registry.py` holds SA private labels (Ritebrand, No Name, PnP, Woolworths, SPAR…) with a chain guard — a "SPAR" token at a Shoprite is not the SPAR brand; unverified entries need chain corroboration. `item_normalizer.py` also extracts `pack_size`/`pack_unit` (g/ml/each, multipacks), `type_key` (brand-independent product key — the apples-to-apples axis with pack size), and check-digit-validated `gtin`. View `house_brand_price_gap` + ontology `housebrandPriceGap`/`privateLabelShare`. Backfill: `python scripts/backfill_item_attributes.py [--apply]`; tests: `python tests/test_brand_attributes.py`.
